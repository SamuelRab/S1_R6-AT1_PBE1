...
const produtoController = {
    /**
     * Retorna os produtos cadastros no banco de dados
     * Rota: GET/ produtos
     * @async
     ...
     * @param {Request} req Objeto da requisição HTTP
     * @param {Response} res Objeto da resposta HTTP
     * @returns {Promise<Array<<Object>} Retorna o objeto contendo os dados do produto pesquisado
     */
    ...
    buscarProdutoPorId: async (req, res) => {
     try {
        ...
        if(!id || !Number.isInteger(id)) {
            return res.status(400).json({message: 'Forneça um identificador com (id) válido'})
        }
        const resultado = await produtoModel.selecionarPorId(id);
        res.status(200).json({message: 'Resultado dos dados listados'});
     } catch (error) {
        console.log(error);
...
descricao = descricao.trim();
            ...
            const novaValor = valor ?? produtoAtual[0].valor;
            const resultado = await produtoModel.alterarProduto(idProduto, novaDescricao, novaValor);
            ...
            console.error(error);
            res.status(500).json({mesage: 'Ocorreu um erro no servidor.', errorMessage: error.message});
        }
    },
